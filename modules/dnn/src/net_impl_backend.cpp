...
OrtPathString toOrtPath(const std::string& utf8Path)
{
    ...
    // MB_ERR_INVALID_CHARS matters: without it, MultiByteToWideChar quietly
    // substitutes U+FFFD for malformed bytes and hands back a path that looks
    // plausible but points nowhere. Failing loudly here is far easier to debug.
    ...
    return wide;
#else
    // ORTCHAR_T is char here, and OpenCV already holds the path as UTF-8.
    return utf8Path;
#endif
}
