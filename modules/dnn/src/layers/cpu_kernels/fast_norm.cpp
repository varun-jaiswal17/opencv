...
// Unchanged -- only mvn_layer.cpp calls this overload, and MVN is not one of the
// engine-new layers this dtype pass covers.
...
// RMSNorm (LayerNorm2's fastNormMeanInvStdDev consumer), and the shared accumulator
// used below: templated on T so CV_64F gets a genuine T=double accumulator instead of
// a float one -- a float accumulator here would defeat the reason a caller asked for
...
// InstanceNorm, BLOCK layout, CV_32F. Extracted verbatim from the pre-existing
// single-type implementation -- untouched logic, including its SIMD paths.
...
// InstanceNorm, BLOCK layout, CV_64F. Scalar (no SIMD) -- same algorithm and the same
// per-channel mean/var/affine structure as fastNormChannelBlockF32 above, generalized
// to T so a double input gets a genuine double accumulator and a genuine double
// affine (alpha/beta), not a float one. Max needed no accumulator (§ MaxPool); this
// does, so unlike MaxPool this is real new arithmetic, not just a new instantiation.
template<typename T>
...
// GroupNorm, BLOCK layout, CV_32F. Extracted verbatim -- untouched logic and SIMD.
...
// GroupNorm, BLOCK layout, CV_64F. Scalar (no SIMD), same structure as
// fastNormGroupBlockF32 above, generalized to T -- see fastNormChannelBlockT for why
// this is genuine new arithmetic rather than a plain new instantiation.
template<typename T>
