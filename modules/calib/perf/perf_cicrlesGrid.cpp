...
// Perf test for the computeRNG path using synthetic point sets, no image I/O needed.
// Tests larger grids where the Delaunay-based RNG gives a measurable speedup over O(N^3).
typedef perf::TestBaseWithParam<cv::Size> CirclesGrid_RNG_Size;

PERF_TEST_P(CirclesGrid_RNG_Size, symmetric_synthetic,
            testing::Values(cv::Size(6, 5), cv::Size(8, 6), cv::Size(10, 8), cv::Size(15, 12), cv::Size(20, 15)))
{
    ...
    TEST_CYCLE()
        findCirclesGrid(cv::Mat(pts), patternSize, centers,
